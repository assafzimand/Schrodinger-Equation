...
def ncc_mismatch_rate(embeddings: np.ndarray, labels_true: np.ndarray,
                      num_classes: int, use_gpu: bool = True) -> Dict[str, np.ndarray]:
    """Compute NCC mismatch rate, assigned classes, and distances in (u,v) space.

    Uses PyTorch for GPU acceleration when available.

    Args:
        embeddings: (N, D) numpy array of embeddings (layer features)
        labels_true: (N,) numpy array of true (u,v)-class indices
        num_classes: bins**2, total number of 2D (u,v) classes
    ...
    Returns:
        Dictionary with:
            ...
            - distances: (N,) numpy array of distances to nearest (u,v) class center
